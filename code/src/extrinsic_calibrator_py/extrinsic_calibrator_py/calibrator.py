...
class ExtrinsicCalibrator(Node):
    ...
    def generate_reliable_transform_between_markers_table(self):
        ...
        self.reliable_transform_between_markers_table = [
            [None for _ in range(self.max_marker_id + 1)]
            for _ in range(self.max_marker_id + 1)
        ]
        for origin_marker_id in range(self.max_marker_id + 1):
            for destination_marker_id in range(self.max_marker_id + 1):
                if (
                    self.path_between_markers_with_cameras_table[origin_marker_id][
                        destination_marker_id
                    ]
                    == None
                ):
                    self.reliable_transform_between_markers_table[origin_marker_id][
                        destination_marker_id
                    ] = None
                else:
                    self.reliable_transform_between_markers_table[origin_marker_id][
                        destination_marker_id
                    ] = self.compose_marker_to_marker_transform(
                        self.path_between_markers_with_cameras_table[origin_marker_id][
                            destination_marker_id
                        ]
                    )
