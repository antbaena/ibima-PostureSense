...
class ExtrinsicCalibrator(Node):
    def __init__(self):
        ...
        info_topics = self.get_parameter('info_topics').get_parameter_value().string_array_value
        frame_ids = self.get_parameter('frame_ids').get_parameter_value().string_array_value

        # Check if the number of cameras is consistent
        if not (len(camera_names) == len(image_topics) == len(info_topics) == len(frame_ids) or 
                len(camera_names) == 0):
            self.get_logger().error("The number of camera names, image topics, info topics and frame ids must be the same and at least 1.")
            self.destroy_node()
            return
        ...
        self.get_logger().info(f"Created {len(self.array_of_cameras)} cameras with the following configurations:")
        for camera in self.array_of_cameras:
            camera:Camera
            self.get_logger().info(f"Camera {camera.camera_id}: {camera.camera_name}, image topic: {camera.image_topic}, info topic: {camera.camera_info_topic}, frame id: {camera.camera_frame_id}")
        # periodically check if all cameras are calibrated        
        self.timer = self.create_timer(5.0, self.check_camera_transforms_callback)
    ...
    def initiate_calibration_routine(self):
        ...
        steps = [
            (self.generate_is_marker_visible_from_camera_table, "Generating marker visibility table"),
            (self.find_central_marker, "Finding central marker"),
            (self.generate_transform_between_markers_table, "Generating marker-to-marker transform table"),
            (self.generate_path_between_markers_table, "Generating paths between markers"),
            (self.generate_reliable_transform_between_markers_table, "Generating reliable marker transforms"),
            (self.generate_camera_to_marker_transform_table, "Generating camera-to-marker transform table"),
            (self.generate_world_to_cameras_transform_table, "Generating world-to-cameras transform table"),
...
# ///////////////////////////////////////////////////////////////////////////////////////////
    ...
    def generate_is_marker_visible_from_camera_table(self):  
        ...
        print(f"Largest marker: {self.largest_marker}")
        ...
        for camera in self.array_of_cameras:
            ...
            for marker_id in seen_markers:
                if marker_id in marker_ids_in_camera:
                    print(f"Camera {camera.camera_name} sees marker {marker_id}")
                    self.is_marker_visible_from_camera_table[marker_id][camera.camera_id] = True


                
        # Display the is_marker_visible_from_camera_table
        display_camera_to_marker_table("Which markers does each camera see:", self.is_marker_visible_from_camera_table, self.array_of_cameras, self.get_logger())
    ...
    def find_central_marker(self):
        ...
        display_camera_to_marker_table("Scores of each camera-marker couple:", self.scores_table, self.array_of_cameras, self.get_logger())
    ...
    def generate_transform_between_markers_table(self):
        ...
        display_marker_to_marker_table(f"Does exist a Transform between two markers through a camera:", self.does_transform_exist_between_markers_table, self.largest_marker, self.get_logger())
    ...
    def generate_camera_to_marker_transform_table(self):
        ...
        for camera in self.array_of_cameras:
            ...
            for marker_id in range(self.largest_marker + 1):
                if self.is_marker_visible_from_camera_table[marker_id][camera.camera_id]:
                    self.camera_to_marker_transform_table[marker_id][camera.camera_id] = camera.markers[marker_id].reliable_transform
                else:
...
# ///////////////////////////////////////////////////////////////////////////////////////////
    ...
    def compose_world_to_camera_transform(self, camera_id):
        ...
        if not array_of_camera_to_world_transforms:
            self.get_logger().error(f"No valid camera-to-world transforms for camera {camera_id}. Returning identity transform.")
            return np.eye(4)
