...
class Camera:
    def __init__(self, node: Node, camera_name: str, camera_id: int, image_topic: str, camera_info_topic: str,
        ...
        self.max_attempt_time = 60.0   # Tiempo máximo (en segundos) para intentar que un marcador se vuelva reliable
        ...
        self.node.get_logger().info(f"Camera {self.camera_name} initialized with ArUco parameters: {aruco_dict_name}, marker length: {marker_length}m.")
    ...
    def image_callback(self, msg):
        ...
        gray_filtered = cv2.GaussianBlur(gray, (5, 5), 0) #TODO:WTF
        ...
        for i, id_arr in enumerate(ids):
            ...
            if marker_id in self.dead_markers:
                self.node.get_logger().debug(f"Camera {self.camera_name}: Marcador {marker_id} marcado como dead, se omite.")
                continue

            corner = corners[i].reshape((4, 2))
            success, rvec, tvec = cv2.solvePnP(self.obj_points, corners[i], self.camera_matrix, self.dist_coeffs)
            if not success:
                continue
            ...
            if distance < 0.1 or area < 10 or distance > 100 or area > 10000:
                self.node.get_logger().warn(f"Camera {self.camera_name}: Marcador {marker_id} descartado por distancia o área fuera de rango.")
                continue
            ...
            if not self._is_marker_valid(distance, area):
                ...
                if marker_id in self.markers:
                    self.node.get_logger().info(f"Camera {self.camera_name}: Marcador {marker_id} removido de la lista de válidos.")
                ...
                if marker_id not in self.markers:
                    self.markers[marker_id] = Marker(marker_id, self.marker_length, timeout=5.0)
                ...
                if self.verbose:
                    ...
                    try:
                        ...
                        self.node.get_logger().error(f"Camera {self.camera_name}: Error publishing image: {e}")
    ...
    def _update_adaptive_thresholds(self, distance: float, area: float, alpha: float = 0.1):
        ...
        self.adaptive_distance_threshold = (1 - alpha) * self.adaptive_distance_threshold + alpha * distance
        self.adaptive_area_threshold = (1 - alpha) * self.adaptive_area_threshold + alpha * area
    ...
    def _update_marker_status(self, current_time: float):
        for marker_id in list(self.markers):
            ...
            ratio = min(marker.variability / self.diff_threshold, 0.5) if self.diff_threshold > 0 else 0
            effective_timeout = self.max_attempt_time * (1 - ratio)
            if marker.is_timed_out():
                self.node.get_logger().warn(f"Camera {self.camera_name}: Marcador {marker_id} timed out, removiendo de tracking.")
                del self.markers[marker_id]
            elif not marker.reliable:
                if marker.is_precise():
                    marker.reliable = True
                    self.node.get_logger().info(f"Camera {self.camera_name}: Marcador {marker_id} ahora es reliable.")
    ...
    def are_all_transforms_precise(self) -> bool:
        if not self.markers:
            self.node.get_logger().warn(f"Camera {self.camera_name}: No se han detectado marcadores.")
            return False

        unreliable_markers = [marker.id for marker in self.markers.values() if not marker.is_precise()]
        ...
        if unreliable_markers:
            ...
            self.node.get_logger().info(f"Camera {self.camera_name}: ¡Todos los marcadores son reliable!")
            return True
